...
const Payment = () => {
  ...
  const fetchPayments = async () => {
    setLoading(true);
    await axios
      .get(`${apiKey}api/paypal/payments?search=${query}&displayMode=${displayMode}`, {
        headers: {
  ...
  useEffect(() => {
    fetchPayments();  // Call the API when either `displayMode` or `query` changes
  }, [displayMode]);
                          ...
                          <tr key={index}>
                            ...
                            <td className={classes}>
                              <Typography
                                ...
                                className="font-normal text-gray-600 dark:text-white"
                              >
                                {new Date(element.payment_date).toLocaleDateString('en-GB', {
